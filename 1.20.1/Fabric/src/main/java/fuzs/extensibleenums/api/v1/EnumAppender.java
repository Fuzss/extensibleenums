...
public final class EnumAppender<T extends Enum<T>> {
     ...
     */
    private static final Map<Class<?>, Class<?>> PRIMITIVE_TYPES = ImmutableMap.<Class<?>, Class<?>>builder()
    ...
    public EnumAppender(Class<T> enumClazz, Class<? extends T> enumConcreteClazz, List<FieldAccess> fields) {
        ...
        this.fields = fields.stream()
                .map(f -> findField(enumClazz, f.ordinal(), f.clazz()))
     ...
     * may only be called once, the builder is cleared afterwards
     *
    ...
    public static <T extends Enum<T>> EnumAppender<T> create(Class<T> enumClazz, Class<?>... clazzes) {
        return new EnumAppender<>(enumClazz, Stream.of(clazzes).map(clazz -> new FieldAccess(0, clazz)).collect(ImmutableList.toImmutableList()));
    }
